...
create table `ydf_flash_promotion`
(
    id         int unsigned not null auto_increment,
    title      varchar(200) not null default '' comment '标题',
    status     tinyint      not null default 1 comment '状态 1=>上线  2=>下线',
  ...
  AUTO_INCREMENT = 1
  DEFAULT CHARSET = utf8mb4 COMMENT ='限时购表';

create table `ydf_flash_promotion_session`
(
    id                 int unsigned not null auto_increment,
    flash_promotion_id int          not null default 0 comment '限时购id',
  ...
  AUTO_INCREMENT = 1
  DEFAULT CHARSET = utf8mb4 COMMENT ='限时购-场次表';

create table `ydf_flash_promotion_product_relation`
(
    id                         int unsigned            not null auto_increment,
    flash_promotion_id         int                     not null default 0 comment '限时购id',
    flash_promotion_session_id int                     not null default 0 comment '限时购id',
    goods_id                   int                     not null default 0 comment '商品id',
    product_id                 int                     not null default 0 comment '货品id',
    flash_promotion_price      decimal(10, 2) unsigned NOT NULL DEFAULT '0.00' COMMENT ' 限时购商品价格 ',
    flash_promotion_total      int comment '商品数量',
    flash_promotion_count      int comment '已购数量',
    flash_promotion_limit      int comment '每人限购数量',
    sort                       int comment '排序 越小越靠前',
    primary key (`id`)
) ENGINE = InnoDB
  AUTO_INCREMENT = 1
  DEFAULT CHARSET = utf8mb4 COMMENT ='限时购-场次商品表';


create table `ydf_money_template`
(
    `id`         int                     NOT NULL AUTO_INCREMENT,
    `money`      decimal(10, 2) unsigned NOT NULL DEFAULT '0.00' COMMENT '充值金额',
    `give`       decimal(10, 2) unsigned NOT NULL DEFAULT '0.00' COMMENT '赠送金额',
    `status`     tinyint(4)              NOT NULL DEFAULT '1' COMMENT '1=>正常， 2=>禁用',
    `sort`       int                     not null DEFAULT '100' COMMENT '排序 越小越靠前',
    `created_at` datetime                         DEFAULT NULL,
    PRIMARY KEY (`id`)
) ENGINE = InnoDB
  DEFAULT CHARSET = utf8mb4 COMMENT ='充值模版表';
...
(
    ...
    `account_money`       decimal(10, 2) unsigned NOT NULL DEFAULT '0.00' COMMENT '账户金额',
    `share_account_money` decimal(10, 2) unsigned NOT NULL DEFAULT '0.00' COMMENT '分销账户余额',
    `saler_info_id`       int(11)                 NOT NULL DEFAULT '0' COMMENT '分销详情id',
#     `unique_code`         varchar(32)             NOT NULL DEFAULT '' COMMENT '身份码',
#     `pid`                 int(11)                 NOT NULL DEFAULT '0' COMMENT '上级id',
#     `ppid`                int(11)                 NOT NULL DEFAULT '0' COMMENT '上上级id',
    `status`              tinyint(4)              NOT NULL DEFAULT '1' COMMENT '1=>正常， 2=>禁用',
    `created_at`          datetime                         DEFAULT NULL,
    `updated_at`          datetime                         DEFAULT NULL,
    `userinfo_at`         datetime                         DEFAULT NULL,
    PRIMARY KEY (`id`)
) ENGINE = InnoDB
  AUTO_INCREMENT = 1
  DEFAULT CHARSET = utf8mb4 COMMENT ='用户表';
/*
alter table  ydf_user add `account_money`   decimal(10, 2) unsigned NOT NULL DEFAULT '0.00' COMMENT '账户金额' after session_key ;
alter table  ydf_user add `share_account_money`   decimal(10, 2) unsigned NOT NULL DEFAULT '0.00' COMMENT '分销账户余额' after account_money ;
*/
